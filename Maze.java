   ...
   public Maze (String fileName) { 
        ...
        for (int i=0; i<image.width(); i++) {
            for (int j=0; j<image.height(); j++){
                if((image.isRed(i,j) && !found)) {                        // the first time that is red
                    startX = i;
                    startY = j;
                    found = true;
                ...
                if ((image.isRed(i,j) && found)) {                        // second time red
                    endX = i;
                    endY = j;
                    image.set(i,j,new Color(0xFF,0xFF,0xFF));
                }

                if(i != 0 ) {                             // ignore the upper and left sides to avoid out of bound.
                    connect(i,j,i-1,j);
                }
                if (j != 0){                              // ignore the upper and left sides to avoid out of bound.
                    connect(i,j,i,j-1);
                }
            }
        }
